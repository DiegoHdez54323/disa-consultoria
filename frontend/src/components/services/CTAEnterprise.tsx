import { ArrowRight, Terminal, CheckCircle2, Server, Database, Activity, Lock, Globe, Layers } from "lucide-react";
...
export const EnterpriseCtaReact = () => {
  ...
  return (
    <div className="relative mt-12 mb-6 mx-4 md:mx-0 overflow-hidden rounded-[2rem] border border-emerald-500/20 bg-card/40 backdrop-blur-md group">
      
      {/* Fondo Animado Sutil */}
      <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-cyan-500/5 group-hover:opacity-100 transition-opacity duration-700" />
      ...
      <div className="relative z-10 p-8 md:p-10 flex flex-col lg:flex-row items-start gap-12">
        ...
        <div className="flex-1 text-left">
          ...
          <h4 className="font-orbitron text-2xl md:text-3xl font-bold text-white mb-4 leading-tight">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">Operaciones Críticas</span>
          </h4>
          
          <p className="text-sm text-muted-foreground mb-6 leading-relaxed max-w-lg">
            Desarrollamos el núcleo digital de tu empresa. Desde <strong>ERPs modulares</strong> hasta <strong>CRMs predictivos</strong>, creamos software que se adapta a tus reglas de negocio, y no al revés.
          </p>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {specs.map((spec, idx) => (
                <div key={idx} className="bg-white/5 border border-white/5 rounded-lg p-3 text-center hover:bg-white/10 transition-colors">
                    <spec.icon className="w-5 h-5 text-emerald-400 mx-auto mb-2" />
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">{spec.label}</div>
          ...
          <div className="flex flex-wrap gap-4">
            <a href="/contacto?servicio=Enterprise-Medida" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-xs font-bold hover:shadow-lg hover:shadow-emerald-500/25 transition-all transform hover:-translate-y-0.5">
        ...
        <div className="hidden lg:block relative w-full max-w-[320px] shrink-0 pt-4">
             ...
             <div className="bg-[#0f172a] border border-white/10 rounded-xl overflow-hidden shadow-2xl relative z-10">
                ...
                <div className="p-4 font-mono text-[11px] h-40 flex flex-col space-y-2.5 bg-black/40">
                    <div className={`transition-opacity duration-300 ${codeLine >= 0 ? 'opacity-100' : 'opacity-30'}`}>
                        <span className="text-emerald-500">➜</span> <span className="text-cyan-300">docker-compose</span> <span className="text-white">up -d --build</span>
                    </div>
                    <div className={`transition-opacity duration-300 ${codeLine >= 1 ? 'opacity-100' : 'opacity-0'} text-gray-400 pl-4`}>
                        Building module: <span className="text-yellow-200">ERP_Core</span>... <span className="text-green-400">Done (0.4s)</span>
                    </div>
                    <div className={`transition-opacity duration-300 ${codeLine >= 2 ? 'opacity-100' : 'opacity-0'} text-gray-400 pl-4`}>
                        Building module: <span className="text-yellow-200">CRM_Sync</span>... <span className="text-green-400">Done (0.2s)</span>
                    </div>
                    <div className={`transition-opacity duration-300 ${codeLine >= 3 ? 'opacity-100' : 'opacity-0'} text-gray-400 pl-4`}>
                        Connecting to <span className="text-blue-300">AWS_RDS</span>... <span className="text-green-400">Connected</span>
                    </div>
                     <div className={`transition-opacity duration-300 ${codeLine >= 4 ? 'opacity-100' : 'opacity-0'}`}>
             ...
             >
                 ...
                 <div>
                     <div className="text-[10px] text-muted-foreground uppercase font-bold">SLA Uptime</div>
            ...
            <motion.div 
             ...
             >
                 ...
                 <div>
                     <div className="text-[10px] text-muted-foreground uppercase font-bold">Data Processed</div>
